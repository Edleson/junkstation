...
app.controller('newsletterController', function ($scope, newsletterService, $rootScope, ngDialog, situacoes) {
	$scope.entities   = [];
	$scope.entity     = {};
	$scope.situacoes  = situacoes;
	var Service       = newsletterService;
	...
	$scope.createOrUpdate = function(entity){
		...
		if(entity._id){
			Service.update(entity).success(function(response, status){
				...
				findAll();
				showGrowl("Notificação !", "Newsletter " + entity.email + " atualizada com sucesso !", 6000);
			}).error(function(response){
				showGrowl(response.header.error.code + " : " + response.header.error.errmsg, response.header.message,  6000);
			});
		}else{
			Service.create(entity).success(function(response, status){
				...
				$scope.entityForm.$setPristine();
				findAll();
				showGrowl("Notificação !", "Newsletter " + entity.email + " criada com sucesso!", 6000);
			}).error(function(response){
				showGrowl(response.header.error.code + " : " + response.header.error.errmsg, response.header.message,  6000);
			});
		}
	...
	$scope.destroy = function(entity){
		ngDialog.openConfirm({
            ...
            Service.destroy(entity).success(function(response, status){
            	showGrowl(null, "Newsletter " + entity.email + " excluído com sucesso !", 6000);
				delete $scope.entity;
				findAll();	
			}).error(function(response){
				showGrowl(response.header.error.code + " : " + response.header.error.errmsg, response.header.message,  6000);	
