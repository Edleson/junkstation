...
app.controller('servicoController', function ($scope, servicoService, $rootScope, ngDialog, situacoes) {
	$scope.entities   = [];
	$scope.entity     = {};
	...
	$scope.createOrUpdate = function(entity){
		...
		if(entity._id){
			Service.update(entity).success(function(response, status){
				...
				findAll();
				showGrowl("Notificação !", "Serviço " + entity.titulo + " atualizada com sucesso !", 6000);
			}).error(function(response){
				showGrowl(response.header.error.code + " : " + response.header.error.errmsg, response.header.message,  6000);
			});
		}else{
			Service.create(entity).success(function(response, status){
				...
				$scope.entityForm.$setPristine();
				findAll();
				showGrowl("Notificação !", "Serviço " + entity.titulo + " criada com sucesso!", 6000);
			}).error(function(response){
				showGrowl(response.header.error.code + " : " + response.header.error.errmsg, response.header.message,  6000);
			});
		}
	...
	$scope.destroy = function(entity){
		ngDialog.openConfirm({
            ...
            Service.destroy(entity).success(function(response, status){
            	showGrowl(null, "Serviço " + entity.titulo + " excluído com sucesso !", 6000);
				delete $scope.entity;
				findAll();	
			}).error(function(response){
				showGrowl(response.header.error.code + " : " + response.header.error.errmsg, response.header.message,  6000);	
