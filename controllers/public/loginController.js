module.exports = function(app) {
    ...
    controller.novasenha = function(req, res, next) {
        var ResponseAPI = createResponseAPI();
        var hash = new User({});
        var id = req.user._id;
        var senhaDigitada = req.body.senha_atual;
        var novaSenha = req.body.password;
        //var senhaAtaul = req.user.local.password;

        if (!req.user.validPassword(senhaDigitada)) {
            var mensage = "<div class='alert-error'>A senha atual incorreta!</div>";
            ResponseAPI.header.url = req.url;
            ResponseAPI.data = mensage;
            res.status(200).json(ResponseAPI);
        } else {
            User.update({ _id: id }, { "local.password": hash.generateHash(novaSenha) }, { multi: false }, function(error, user) {
                if (error) {
                    ResponseAPI.header.status = 500;
                    ResponseAPI.header.url = req.url;
                    ResponseAPI.header.message = "Não foi possível alterar a senha!";
